...
const fetchTokenPriceData = async (
  ...
  try {
    ...
    Object.keys(prices).forEach((priceKey) => {
      ...
      if (timestamp) {
        const tokenPriceIndex = tokenPrices.findIndex(
          (tokenPrice) => tokenPrice.timestamp === timestamp
        );
        if (tokenPriceIndex >= 0) {
          const { derivedBNB } = tokenPrices[tokenPriceIndex];
          tokenPrices[tokenPriceIndex].priceUSD =
            parseFloat(
              prices[priceKey]?.[`${mainToken.toLowerCase()}Price`] ?? 0
            ) * derivedBNB;
        }
      }
    });
    ...
    const sortedTokenPrices = orderBy(tokenPrices, (tokenPrice) =>
